...
class MapViewer:
    ...
    def setup_viewer(self):
        ...
        if len(monitors) > 1:
            m = monitors[1]
            width, height, x, y = m.width, m.height, m.x, m.y
        else:
            # single monitor fallback: use 540x960 centered on primary monitor
            m = monitors[0]
...
''''
    Grid toggle: cycle states OFF -> LIGHT -> DARK using an int var.
    Image zoom: add zoom factor with +/- buttons; center maintained by current centering math; "add black borders" is default (image drawn centered on black canvas).
    Brightness: add factor and apply ImageEnhance.Brightness before resizing.
    Single-monitor fallback: create a viewer window sized 540x960 centered on primary monitor if only one monitor.

Changes to add/replace:

A — Add imports and constants at top (replace existing imports block):
python
...
B — In MapViewer.init, add new control vars (replace after self.mirrored = False):
python
...
C — Modify setup_ui: replace the existing GRID button creation with the new handlers and add zoom/brightness adjusters under grid controls. Replace the grid button block with:
python
...
Then, below make_adjuster calls, add two small adjuster frames for Zoom and Brightness (keep compact):
python
...
D — Add the following new methods inside MapViewer (anywhere among methods):
python
...
E — Modify setup_viewer single-monitor behavior. Replace that method with:
python
...
F — Modify redraw to apply brightness, zoom, and grid states. Replace the existing redraw method body with:
python
